...
def repeats(our_str):
    ...
    new_str = ""
    dict = {}
    for i in our_str:  # Пробегаем по каждой букве слова
        if dict.get(i) is None:  # Если нет буквы, то добавляем в словарь ключ и его значение = 1
            dict[i] = 1
        else:
            dict[i] += 1
        new_str += i + "_" + str(dict[i])  # Собираем строку для ответа
    return new_str
