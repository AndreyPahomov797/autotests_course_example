...
def multiplication_chain(num):
    ...
    count_multy = 0
    while len_num != 1:
        count_multy = count_multy + 1  # Каждый раз, если количество цифр результата пермножения больше одного плюсуем 1
        multi = 1
        str_num = str(num)
        for i in range(len_num):
            multi = multi * int(str_num[i])
        num = multi  # После перемножения num уже результат последнего перемножения
        len_num = len(str(num))
    return count_multy
