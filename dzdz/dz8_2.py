...
class Trigon:
    ...
    def __init__(self, *args):
        """Происходит проверка входных данных длин сторон треугольника и выводится ошибка с описанием"""
        self.args = args
        if len(self.args) == 3:
            if type(self.args[0]) != int or  type(self.args[1]) != int or type(self.args[2]) != int:
                raise TypeError('Стороны должны быть числами')
            if self.args[0] <= 0 or  self.args[1] <= 0 or self.args[2] <= 0:
               raise ValueError('Стороны должны быть положительными')
            if self.args[0]+self.args[1] <= self.args[2] or self.args[1]+self.args[2] <= self.args[0] or self.args[2]+self.args[0] <= self.args[1]:
