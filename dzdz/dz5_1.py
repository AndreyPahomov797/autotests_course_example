...
def letter_stat(our_str):
    ...
    letters_dict = {}
    for i in range(len(our_str)):  # Пробегает по всему слову по буквам
        summ = 0
        for k in range(len(our_str)):  # Пробегает по всему слову и считает сколько повторяется i-ый символ слова
            if our_str[k] == our_str[i]:
                summ = summ + 1
        letters_dict.update({our_str[i]: summ})  # Добавляет в пустой словарь тождественно равный
    return letters_dict
